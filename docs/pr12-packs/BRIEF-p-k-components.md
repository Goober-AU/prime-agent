...
Worklist for your pack (read it FIRST): docs/pr12-packs/p-k-components.md
Owned-file list: docs/pr12-packs/p-k-components.files
...
6. Rust toolchain is PRIVATE. Always run cargo through this environment:
     ...
     CARGO_BUILD_JOBS=4 CARGO_INCREMENTAL=0 CARGO_TARGET_DIR=.port-env/target \
       cargo check -p pi-coding-agent --tests --message-format=json > .port-env/tmp/w-p-k-components-runN.jsonl 2> .port-env/tmp/w-p-k-components-runN.err
