...
import { getOptimusLogo } from "../src/themes/optimus-logo.js";
...
describe("PrimeOnboardingSplashComponent", () => {
	...
	it("renders a minimal first-run login action", () => {
		...
		expect(output).toContain("Welcome to OPTIMUS");
		...
		expect(output).toContain(getOptimusLogo(100, 27)[0].trim());
	...
	it("animates the splash at an interactive cadence", () => {
		...
		expect(secondRender).toContain("Welcome to OPTIMUS");
	...
	it("centers stacked content in narrow terminals", () => {
		...
		const logoLine = rendered.find((line) => line.includes(getOptimusLogo(60, 31)[0].trim()));
		const brandLine = rendered.find((line) => line.includes("Welcome to OPTIMUS"));
