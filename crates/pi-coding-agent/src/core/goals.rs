...
pub fn create_goal_context_message(
    ...
    Ok(CustomMessage {
        ...
        display: true,
        details: Some(
            serde_json::to_value(GoalContextDetails {
                kind,
                goal_id: goal.goal_id.clone(),
                objective,
                status: goal.status,
...
mod tests {
    ...
    fn computes_token_deltas_from_usage() {
        assert_eq!(
            goal_token_delta_for_usage(GoalUsage {
                input: 3.0,
                output: 4.0
            }),
            7.0
    ...
    fn recognizes_persisted_goal_state() {
        ...
        assert!(!is_persisted_goal_state(
            &serde_json::json!({"active": true, "status": "weird", "tokensUsed": 0, "timeUsedSeconds": 0, "continuationsUsed": 0})
        ));
        assert!(!is_persisted_goal_state(
            &serde_json::json!({"active": true, "status": "active", "tokensUsed": "0", "timeUsedSeconds": 0, "continuationsUsed": 0})
        ));
    }
    ...
    fn creates_goal_context_messages_with_xml_escaped_objectives() {
        ...
        let message =
            create_goal_context_message(&goal, GoalContextKind::Continuation, None).unwrap();
        ...
        assert_eq!(
            message.details.as_ref().unwrap()["kind"],
            serde_json::json!("continuation")
        );
        assert_eq!(
            message.details.as_ref().unwrap()["objective"],
            serde_json::json!("use <a> & <b>")
        );
        let with_images = create_goal_context_message(
            &goal,
        ...
        assert!(blocks[0]["text"]
            .as_str()
            .unwrap()
            .contains("- status: budget_limited"));
        let missing =
            create_goal_context_message(&empty_goal_state(), GoalContextKind::Continuation, None);
        assert_eq!(
            missing.unwrap_err(),
            "Cannot create goal context without an objective."
        );
    }
    ...
    fn objective_updated_prompt_uses_the_untrusted_objective_block() {
        ...
        let message =
            create_goal_context_message(&goal, GoalContextKind::ObjectiveUpdated, None).unwrap();
        let text = message.content.as_str().unwrap();
