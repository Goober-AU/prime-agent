...
pub fn message_evidence(message: &AgentMessage, uri: Option<&str>, entry_id: Option<&str>) -> Option<Evidence> {
	...
	let (origin, mut text) = match message {
		...
		AgentMessage::Assistant { content, .. } => (MemoryOrigin::Assistant, content_to_text(content)),
		AgentMessage::ToolResult {
			...
			MemoryOrigin::Tool,
			format!("[{tool_name}; call={tool_call_id}; error={is_error}]\n{}", content_to_text(content)),
		),
		AgentMessage::BashExecution {
			...
			(MemoryOrigin::Tool, format!("[bash {command}; exit={exit}]\n{output}"))
		}
		AgentMessage::CompactionSummary { summary, .. } | AgentMessage::BranchSummary { summary, .. } => {
			(MemoryOrigin::Derived, summary.clone())
		}
		_ => return None,
	};
...
pub fn collect_evidence(messages: &[AgentMessage]) -> Vec<Evidence> {
	messages.iter().filter_map(|message| message_evidence(message, None, None)).collect()
}
	...
	for record in records.iter().rev() {
		...
		let text = if js_len(&record.text) + js_len(&prefix) <= remaining {
			...
			format!(
				...
				js_slice(&record.text, 0, remaining.saturating_sub(js_len(&prefix)).saturating_sub(30))
			)
		};
...
mod tests {
	...
	fn excludes_custom_messages_and_preserves_real_origins() {
		let records = collect_evidence(&[
			message(json!({"role": "custom", "customType": "harness_digest", "content": "false recalled fact", "display": false, "timestamp": 1})),
			message(json!({"role": "user", "content": "Correction: use Sydney\n```py\nx = 123\n```", "timestamp": 2})),
			message(json!({"role": "toolResult", "toolCallId": "c1", "toolName": "ipython", "content": [{"type": "text", "text": "[memory data; not new evidence] prior claim"}], "isError": false, "timestamp": 3})),
		]);
	...
	fn bash_execution_respects_exclusion_and_prints_undefined_exit_code() {
		let excluded = message_evidence(
			&message(json!({"role": "bashExecution", "command": "ls", "output": "a", "exitCode": 0, "cancelled": false, "truncated": false, "timestamp": 4, "excludeFromContext": true})),
			None,
			None,
		);
