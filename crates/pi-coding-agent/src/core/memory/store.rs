...
use crate::core::refinement::refinement::{
	apply_refinement_proposal, ApplyRefinementOptions, HarnessRefinementEvent, HarnessScope, HarnessState,
	RefinementProposal, RefinementResult,
};
...
pub struct MemoryDocument {
	pub schema: f64,
	pub entries: IndexMap<String, IndexMap<String, crate::core::refinement::refinement::HarnessEntry>>,
...
pub fn empty_document(project_id: &str) -> MemoryDocument {
	let mut entries: IndexMap<String, IndexMap<String, crate::core::refinement::refinement::HarnessEntry>> =
		IndexMap::new();
...
fn safe_integer(value: Option<&Value>) -> Option<i64> {
	match value {
		Some(Value::Number(number)) => number.as_f64().filter(|value| value.fract() == 0.0).map(|value| value as i64),
		_ => None,
	}
}
...
pub fn validate_settings(value: &Value) -> Result<PartialMemorySettings, String> {
	...
	match source.get("shared") {
		...
		Some(Value::Object(_)) => {
			...
			let loopback = ["127.0.0.1", "[::1]", "localhost"].contains(&parsed.host_str().unwrap_or(""));
...
impl MemoryStore {
	pub fn new(agent_dir: &str, project: ProjectIdentity) -> Result<MemoryStore, String> {
		...
		if !host_path.exists() {
			let _lock = super::acquire_lock_sync(&root.to_string_lossy(), super::MemoryLockRetries {
				stale_ms: 10_000,
				retries: 0,
				min_timeout_ms: 0,
				max_timeout_ms: 0,
		...
		let host_id = host
			.get("id")
			.and_then(Value::as_str)
			.filter(|value| regex::Regex::new(r"^[a-f0-9-]{36}$").unwrap().is_match(value))
	...
	pub fn settings(&self) -> MemorySettings {
		...
		let global_memory = global.get("memory").cloned().unwrap_or(Value::Object(JsonMap::new()));
		apply_settings(&mut settings, &validate_settings(&global_memory).unwrap_or_default());
		if local_file.exists() {
			if let Ok(value) = read_json(&local_file.to_string_lossy()) {
				apply_settings(&mut settings, &validate_settings(&value).unwrap_or_default());
			}
		}
		settings
	}
	...
	pub fn backup(&self, doc: Option<MemoryDocument>) -> Result<String, String> {
		...
		let id = format!("backup_{}_{}", doc.memory.revision, &digest[..16.min(digest.len())]);
	...
	pub async fn restore(&self, id: &str) -> Result<i64, String> {
		if !regex::Regex::new(r"^backup_\d+_[a-f0-9]{16}$").unwrap().is_match(id) {
			return Err("Invalid backup ID".to_string());
		}
		let id = id.to_string();
		self.exclusive(move || {
			let path = Path::new(&self.dir).join("backups").join(format!("{id}.json"));
			...
			write_json(&self.path, &serde_json::to_value(&restored).unwrap_or(Value::Null))?;
	...
	pub async fn apply(
		...
		self.exclusive(move || {
			...
			if let Some(prior) = doc.memory.events.get(&options.event_id) {
				...
				let result = doc.memory.history.iter().find(|item| item.id == options.event_id);
				return match result {
			...
			if !regex::Regex::new(r"^[A-Za-z0-9_-]{1,160}$").unwrap().is_match(&options.event_id) {
				return Err("Invalid event ID".to_string());
			}
			let mut edits = Vec::new();
			for edit in &proposal.edits {
				if let Some(id) = &edit.id {
					if !regex::Regex::new(r"^[A-Za-z0-9_-]{1,160}$").unwrap().is_match(id)
				...
				let before = edit
					.id
					.as_ref()
					.and_then(|id| doc.entries.get(&edit.kind).and_then(|bucket| bucket.get(id)))
					.cloned();
				...
				let metadata_host = metadata.get("hostId").and_then(Value::as_str).map(str::to_string);
						...
						.map(|bucket| {
							bucket.values().find(|entry| {
								entry.content.trim() == content.trim()
									&& entry.metadata.get("hostId").and_then(Value::as_str).unwrap_or("")
										== host_id
				...
				metadata.insert("projectId".to_string(), Value::String(self.project.id.clone()));
			...
			candidate.memory.events.insert(options.event_id.clone(), fingerprint);
			candidate.memory.history.push(applied.clone());
			self.backup(Some(doc))?;
			write_json(&self.path, &serde_json::to_value(&candidate).unwrap_or(Value::Null))?;
			Ok(applied)
		})
		.await
	}

	pub async fn rollback(&self, id: &str, expected_revision: i64) -> Result<RefinementResult, String> {
		let doc = self.read()?;
		let result = doc
			.memory
			.history
		...
		for edit in result.applied_edits.iter().rev() {
			...
			match &edit.before {
				Some(before) => {
					...
					next.action = if edit.after.is_some() { "update" } else { "create" }.to_string();
...
fn merge_settings_patch(target: &mut JsonMap, patch: &PartialMemorySettings) {
	...
	set("maxRecallChars", patch.max_recall_chars.map(|value| Value::from(value)));
	set("maxRecallEntries", patch.max_recall_entries.map(|value| Value::from(value)));
	set(
		"maxExtractionTokens",
		patch.max_extraction_tokens.map(|value| Value::from(value)),
	);
	set("maxImportBytes", patch.max_import_bytes.map(|value| Value::from(value)));
	set(
		"maxImportChunkChars",
	...
	set(
		...
		patch.max_import_chunks_per_run.map(|value| Value::from(value)),
	);
...
pub fn memory_source_from_value(value: &Value) -> Option<MemorySource> {
	...
	Some(MemorySource {
		...
		uri: record.get("uri").and_then(Value::as_str).map(str::to_string),
		revision: record.get("revision").and_then(Value::as_str).map(str::to_string),
		project_path: record.get("projectPath").and_then(Value::as_str).map(str::to_string),
	})
}
...
mod tests {
	...
	fn empty_document_matches_the_typescript_shape() {
		...
		assert_eq!(value["memory"]["projectId"], serde_json::json!("project_test"));
	...
	fn serializes_competing_revisions_and_deduplicates_retries() {
		...
		assert_eq!(stale, "Memory changed; review current revision before applying");
	...
	fn backs_up_history_validates_restore_and_preserves_delivery_receipts() {
		...
		assert_eq!(document.memory.events.get("op_one").map(String::as_str), Some(document.memory.events["op_one"].as_str()));
		let error = runtime.block_on(store.restore("nope")).expect_err("invalid id");
	...
	fn rolls_back_a_committed_refinement_and_restores_host_isolation() {
		...
		let result = runtime
			...
			.unwrap();
		let rolled = runtime.block_on(store.rollback(&result.id, 1)).expect("rollback");
	...
	fn keeps_host_facts_private_and_remaps_sources() {
		...
		host_proposal.edits[0].metadata = Some(serde_json::json!({"projectReusable": true}).as_object().unwrap().clone());
		runtime
			.block_on(store.apply(
				&host_proposal,
				ApplyOptions {
			...
			.unwrap();
		let document = store.read().unwrap();
		let entry = document.entries.get("memory").unwrap().get("host_only").unwrap();
		assert_eq!(entry.metadata.get("hostId").and_then(Value::as_str), Some(store.host_id.as_str()));
		assert_eq!(entry.metadata.get("projectId").and_then(Value::as_str), Some("project_test"));
	...
	fn validates_settings_limits_and_shared_endpoints() {
		assert_eq!(validate_settings(&serde_json::json!({})).unwrap().recall, None);
		assert_eq!(
		...
		assert_eq!(
			validate_settings(&serde_json::json!({"shared": {"url": "http://example.com", "tokenFile": "t"}})).unwrap_err(),
