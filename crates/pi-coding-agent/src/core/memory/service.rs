...
use crate::core::kernel::shared::{BoxFuture, HostRequestHandler, HostRequestHandlers, KernelError};
use crate::core::refinement::refinement::{
	get_global_harness_state_dir, get_local_harness_state_dir, load_harness_state, normalize_refinement_proposal,
	HarnessScope,
};
...
use super::search::{freshness, recall_memory, search_memory, MemoryHit, MemoryScope, RecallResult, SearchCorpus};
...
pub fn get_agent_dir() -> String {
	...
	dirs::home_dir()
		.map(|home| home.join(".prime").join("agent").to_string_lossy().to_string())
...
impl MemoryService {
	pub fn new(cwd: &str, agent_dir: &str, session_artifact_dir: Option<String>) -> Result<MemoryService, String> {
	...
	pub fn search(&self, query: &str, include_inactive: bool) -> Vec<MemoryHit> {
		let mut extra: Vec<SearchCorpus> = vec![
			SearchCorpus {
				state: load_harness_state(&get_global_harness_state_dir(&self.store.agent_dir), HarnessScope::Global),
		...
		let revision = || -> Result<i64, String> {
			match payload.get("revision") {
				Some(Value::Number(number)) => number
					.as_f64()
					.filter(|value| value.fract() == 0.0 && *value >= 0.0 && *value <= 9007199254740991.0)
		...
		match action {
			...
			"read" => {
				...
				if let Some(map) = value.as_object_mut() {
					map.insert(
						"freshness".to_string(),
						Value::String(freshness(&hit.sources, Some(&self.store.project.root)).as_str().to_string()),
					);
				}
				Ok(value)
			}
			...
			"bind" => {
				...
				let identity = project_identity(&self.store.project.root, &self.store.agent_dir, Some(&project_id))?;
			...
			"apply" => {
				...
				let raw_edits = record(&raw_proposal)?.get("edits").and_then(Value::as_array).cloned();
				match raw_edits {
			...
			"history" => Ok(serde_json::to_value(self.store.read()?.memory.history).unwrap_or(Value::Null)),
			"rollback" => {
				let id = string("id")?;
			...
			"import_run" => {
				let extract = extract.ok_or_else(|| {
					"Use /memory import-run <id> to extract with the session's selected model".to_string()
				})?;
	...
	fn validate_sources(&self, value: Option<&Value>) -> Result<Vec<MemorySource>, String> {
		...
		for raw in values {
			...
			if !matches!(source.get("id"), Some(Value::String(_)))
				...
				|| !regex::Regex::new(r"^[a-f0-9]{64}$").unwrap().is_match(sha256)
			...
			sources.push(MemorySource {
				...
				uri: source.get("uri").and_then(Value::as_str).map(str::to_string),
				revision: source.get("revision").and_then(Value::as_str).map(str::to_string),
				project_path: source.get("projectPath").and_then(Value::as_str).map(str::to_string),
			});
		}
		Ok(sources)
	}
...
pub type MemoryExtractorFactory = Arc<dyn Fn(MemoryService) -> BoxFuture<'static, Result<MemoryExtractor, String>> + Send + Sync>;
	...
	let handler: HostRequestHandler = Arc::new(move |payload: Value| {
		...
		Box::pin(async move {
			let action = payload
				...
				.to_string();
			let service = MemoryService::new(&cwd, agent_dir.as_deref().unwrap_or(&get_agent_dir()), session_artifact_dir)
			...
			if action == "import_run" {
				...
				extract = Some(factory(service.clone()).await.map_err(KernelError::message)?);
			}
