...
pub fn freshness(refs: &[MemorySource], project_root: Option<&str>) -> MemoryFreshness {
	...
	for source in refs {
		let uri = match (&source.origin, &source.uri) {
			(crate::core::memory::evidence::MemoryOrigin::File, Some(uri)) if uri.starts_with("file:") => uri,
			_ => continue,
		};
		...
		if let (Some(root), Some(project_path)) = (project_root, source.project_path.as_ref()) {
			...
			match relative {
				Some(relative) if !relative.starts_with("..") && !Path::new(&relative).is_absolute() => {
					path = resolved;
				}
...
pub fn search_memory(
	...
	for SearchCorpus { state, scope: base_scope } in &corpus {
			...
			for entry in bucket.values() {
				...
				if !include_inactive
					&& (entry.metadata.contains_key("supersededBy")
						|| entry.metadata.get("status").and_then(Value::as_str) == Some("superseded")
						|| entry.metadata.get("detached").is_some())
				{
					continue;
				}
				let title_set: HashSet<String> = words(&format!("{} {} {}", entry.title, entry.path, entry.id))
					.into_iter()
					.collect();
				...
				let scope = if host_id.is_some() { MemoryScope::Host } else { *base_scope };
...
pub fn recall_memory(hits: &[MemoryHit], settings: &MemorySettings, project_root: Option<&str>) -> RecallResult {
	...
	for hit in hits {
		...
		let content = if hit.entry.content.chars().count() as i64 <= space {
			...
			format!(
				...
				hit.entry.content.chars().take(space as usize).collect::<String>()
			)
		};
	...
	let text = if ids.is_empty() { String::new() } else { lines.join("\n") };
...
mod tests {
	...
	fn searches_beyond_overview_limits_across_allowed_stores() {
		...
		let runtime = runtime();
		for index in 0..20 {
			let proposal = crate::core::refinement::refinement::normalize_refinement_proposal(&serde_json::json!({
		...
		state.entries.get_mut("memory").unwrap().insert("global".to_string(), global);
		save_harness_state(
	...
	fn detects_changed_source_files_and_skips_missing_ones() {
		...
		assert_eq!(freshness(std::slice::from_ref(&source), None), MemoryFreshness::Current);
		std::fs::write(&file, "changed").unwrap();
		assert_eq!(freshness(std::slice::from_ref(&source), None), MemoryFreshness::Stale);
		std::fs::remove_file(&file).unwrap();
		assert_eq!(freshness(std::slice::from_ref(&source), None), MemoryFreshness::Missing);
