...
pub type MemoryExtractor =
	Arc<dyn Fn(Vec<Evidence>) -> Pin<Box<dyn Future<Output = Result<ExtractionResult, String>> + Send>> + Send + Sync>;
...
impl MemoryJobs {
	pub fn new(store: MemoryStore) -> MemoryJobs {
		let dir = Path::new(&store.dir).join("jobs").to_string_lossy().to_string();
		MemoryJobs { dir, store }
	}

	fn path(&self, id: &str) -> Result<String, String> {
		if !regex::Regex::new(r"^import_[a-f0-9]{24}$").unwrap().is_match(id) {
	...
	pub async fn prepare(&self, source: &str) -> Result<ImportJob, String> {
		...
		let raw = std::fs::read_to_string(source)
			.map_err(|_| "Selected session exceeds import size limit or is not a file".to_string())?;
			...
			.exclusive(move || {
				...
				for (index, line) in lines.iter().enumerate() {
					...
					let evidence = match message {
						Some(message) => {
							let message: AgentMessage = serde_json::from_value(message).ok().unwrap_or(AgentMessage::Other);
							let uri = format!(
								"{}#L{}",
								url::Url::from_file_path(Path::new(&input_path))
									.map(|url| url.to_string())
									.unwrap_or_else(|_| format!("file://{}", input_path.replace('\\', "/"))),
								index + 1
							);
							let entry_id = row.get("id").and_then(Value::as_str).map(str::to_string);
							message_evidence(&message, Some(&uri), entry_id.as_deref())
						}
						None => None,
					};
					let evidence = match evidence {
						Some(evidence) if evidence.origin != super::evidence::MemoryOrigin::Derived => evidence,
						_ => {
					...
					let text_budget = chunk_budget as i64 - js_len(&serialize_evidence(&[reference], 80_000)) as i64 - 64;
					if text_budget < 1 {
						return Err(
							"Source reference exceeds chunk budget; increase maxImportChunkChars".to_string()
						);
					}
					...
					while offset < characters.len() {
						let part_text: String = characters[offset..(offset + text_budget).min(characters.len())]
							.iter()
							.collect();
						let part = Evidence {
						...
						let fits = match chunks.last() {
							Some(previous) => {
								...
								js_len(&serialize_evidence(&combined, usize::MAX / 4)) <= chunk_budget
							}
							None => false,
						};
						if fits {
	...
	pub async fn run(
		...
		if matches!(job.status, ImportJobStatus::Applied | ImportJobStatus::Preview) {
			return Ok(job);
		}
	...
	pub async fn apply(&self, id: &str, expected_revision: i64) -> Result<ImportJob, String> {
		...
		if job.status != ImportJobStatus::Preview {
			return Err("Import must finish extraction and be previewed before applying".to_string());
		}
		if job.accepted_proposal.is_none() {
					...
					!current
						...
						.map(|bucket| {
							bucket.values().any(|entry| {
								entry.content.trim() == content && !entry.metadata.contains_key("supersededBy")
							})
						})
						.unwrap_or(false)
				})
			...
			job.accepted_proposal = Some(RefinementProposal {
				...
				expected_outcome: "Recover selected project knowledge with original evidence".to_string(),
				edits: new_edits,
			});
...
mod tests {
	...
	fn preserves_raw_code_and_reports_coverage() {
		...
		assert_eq!(std::fs::read_to_string(&input_path).unwrap(), std::fs::read_to_string(&source).unwrap());
	...
	fn rejects_invalid_session_json_and_oversized_files() {
		...
		let missing = runtime
			...
			.expect_err("missing");
		assert_eq!(missing, "Selected session exceeds import size limit or is not a file");
		std::fs::remove_dir_all(&root).ok();
	}
	...
	fn resumes_failed_chunks_and_requires_a_completed_preview() {
		...
		let failing: MemoryExtractor = Arc::new(|_| Box::pin(async { Err("extractor down".to_string()) }));
		let error = runtime.block_on(jobs.run(&job.id, failing, None)).expect_err("failed");
		...
		let apply_error = runtime.block_on(jobs.apply(&job.id, 0)).expect_err("not previewed");
		assert_eq!(apply_error, "Import must finish extraction and be previewed before applying");
		let done = runtime.block_on(jobs.run(&job.id, extractor(), None)).expect("run");
		...
		let again = runtime.block_on(jobs.run(&job.id, extractor(), None)).unwrap();
	...
	fn limits_extraction_calls_per_invocation() {
		...
		let job = runtime.block_on(jobs.prepare(&path.to_string_lossy())).unwrap();
		...
		let first = runtime.block_on(jobs.run(&job.id, extract.clone(), None)).unwrap();
	...
	fn applies_only_creates_and_skips_existing_content() {
		...
		let source = session_file(&root);
		let job = runtime.block_on(jobs.prepare(&source)).unwrap();
		runtime.block_on(jobs.run(&job.id, extractor(), None)).unwrap();
	...
	fn rejects_imports_that_are_not_memory_creates() {
		...
		let error = runtime.block_on(jobs.apply(&job.id, 0)).expect_err("only creates");
