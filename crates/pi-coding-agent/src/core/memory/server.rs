...
pub fn handle_request(
    ...
    let state = if path.exists() {
        match read_json(&path.to_string_lossy()).and_then(|value| {
    ...
    if method == "POST" {
        ...
        if let Some(receipt) = receipt {
            ...
            for entry in &write.entries {
                let id_ok = regex::Regex::new(r"^[A-Za-z0-9_-]{1,160}$")
                    .unwrap()
                    .is_match(&entry.id)
                    && !["__proto__", "constructor", "prototype"].contains(&entry.id.as_str());
                let metadata_project = entry.metadata.get("projectId").and_then(Value::as_str);
            ...
            if serde_json::to_string(&staged)
                .map(|body| body.len())
                .unwrap_or(usize::MAX)
                > MAX_DOCUMENT_BYTES
            {
...
pub fn validate_server_arguments(
    dir: Option<&str>,
    token_file: Option<&str>,
    port_text: Option<&str>,
) -> Result<(String, String, u16), String> {
    let port_text = port_text.unwrap_or("8799");
    let port: i64 = port_text
        .parse()
        .map_err(|_| "Usage: node server.js <state-directory> <token-file> [port]".to_string())?;
...
mod tests {
    ...
    use crate::core::memory::store::{ApplyOptions, MemoryStore};
    ...
    fn fixture() -> (String, std::path::PathBuf) {
        ...
        std::fs::write(
            ...
            serde_json::to_string(&store.read().unwrap()).unwrap(),
        )
        .unwrap();
    ...
    fn applies_a_shared_write_and_writes_a_backup() {
        ...
        assert!(
            document["entries"]["memory"]["one"]["content"] == serde_json::json!("Content one")
        );
