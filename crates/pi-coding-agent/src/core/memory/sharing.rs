...
impl MemorySharing {
	pub fn new(store: MemoryStore) -> MemorySharing {
		let path = Path::new(&store.dir).join("shared.json").to_string_lossy().to_string();
	...
	pub async fn queue(&self, ids: &[String], remove: Vec<String>) -> Result<SharedWrite, String> {
		...
		self.store
			.exclusive(move || {
				...
				write_json(&sharing.path, &serde_json::to_value(&cached).unwrap_or(Value::Null))?;
				Ok(write)
			})
			.await
	}
		...
		store
			.exclusive_async(move || {
				...
				async move {
					...
					let outcome = request_shared_state(&config.url, &store.project.id, &token, &mut cached).await;
					match outcome {
						Ok(()) => {}
						Err(error) => {
		...
		async move {
			...
			if !response.status().is_success() {
				return Err(if response.status().as_u16() == 409 {
					"Shared revision conflict; inspect remote state and requeue explicitly".to_string()
				} else {
...
pub fn validate_shared_write(value: &Value) -> Result<SharedWrite, String> {
	...
	let id_ok = data
		...
		.map(|id| regex::Regex::new(r"^share_[a-f0-9]{32}$").unwrap().is_match(id))
		.unwrap_or(false);
	...
	for id in remove.unwrap() {
		match id.as_str() {
			Some(id) if regex::Regex::new(r"^[A-Za-z0-9_-]{1,160}$").unwrap().is_match(id) => {}
...
mod tests {
	...
	fn reports_offline_sync_and_short_tokens() {
		...
		assert_eq!(error, "Shared memory token must contain at least 32 characters");
	...
	fn validates_shared_writes() {
		...
		let bad_id = serde_json::json!({"id": "share_zz", "revision": 0, "entries": [], "remove": []});
		assert_eq!(validate_shared_write(&bad_id).unwrap_err(), "Invalid shared write");
		let negative = serde_json::json!({"id": format!("share_{}", "a".repeat(32)), "revision": -1, "entries": [], "remove": []});
		assert_eq!(validate_shared_write(&negative).unwrap_err(), "Invalid shared write");
		...
		assert_eq!(validate_shared_write(&too_many).unwrap_err(), "Invalid shared write");
		...
		assert_eq!(validate_shared_write(&bad_removal).unwrap_err(), "Invalid removal ID");
	}
