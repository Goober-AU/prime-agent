...
pub fn parse_skill_block(text: &str) -> Option<ParsedSkillBlock> {
	...
	Some(ParsedSkillBlock {
		name: captures.get(1).map(|value| value.as_str()).unwrap_or("").to_string(),
		location: captures.get(2).map(|value| value.as_str()).unwrap_or("").to_string(),
		content: captures.get(3).map(|value| value.as_str()).unwrap_or("").to_string(),
		user_message,
	})
}
...
mod tests {
	...
	fn parses_a_skill_block_without_a_user_message() {
		let parsed = parse_skill_block("<skill name=\"a\" location=\"b\">\nBody\n</skill>").expect("parsed");
