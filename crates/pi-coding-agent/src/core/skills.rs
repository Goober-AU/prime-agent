...
impl IgnoreMatcher {
	...
	pub fn ignores(&self, path: &str) -> bool {
		...
		for rule in &self.rules {
			let target = if rule.directory_only { format!("{path}/") } else { path.to_string() };
...
fn is_valid_python_import_name(name: &str) -> bool {
	regex::Regex::new(r"^[A-Za-z_][A-Za-z0-9_]*$").unwrap().is_match(name)
}
...
fn load_skill_from_file(file_path: &str, source: &str) -> LoadedSkill {
	...
	if frontmatter.description.as_deref().map(|value| value.trim().is_empty()).unwrap_or(true) {
		return LoadedSkill {
			skill: None,
			diagnostics,
		};
...
pub fn format_skills_for_prompt(skills: &[Skill]) -> String {
	...
	for skill in visible {
		let (base, kind, python_import) = match skill {
			...
			Skill::Python(value) => (&value.base, SkillKind::Python, Some(value.python.import_name.clone())),
		};
		...
		lines.push(format!("    <description>{}</description>", escape_xml(&base.description)));
		lines.push(format!("    <location>{}</location>", escape_xml(&base.file_path)));
		lines.push("  </skill>".to_string());
	}
...
fn resolve_skill_path(path: &str, cwd: &str) -> String {
	...
	if Path::new(&normalized).is_absolute() {
		...
		Path::new(cwd).join(normalized).to_string_lossy().to_string()
	}
}
...
pub fn load_skills(options: &LoadSkillsOptions) -> LoadSkillsResult {
		...
		for skill in result.skills {
			...
			match skill_map.get(&name) {
				Some(existing) => collision_diagnostics.push(ResourceDiagnostic {
					diagnostic_type: crate::core::diagnostics::RESOURCE_DIAGNOSTIC_COLLISION.to_string(),
				...
				None => {
					...
					if let Skill::Python(python) = &skill {
						match python_import_map.get(&python.python.import_name) {
							...
							None => {
								python_import_map.insert(python.python.import_name.clone(), skill.clone());
							}
						}
					}
				}
	...
	if options.include_defaults {
		...
		let project = load_skills_from_dir_internal(&project_skills_dir, "project", true, None, None);
		add_skills(
			project,
			&mut skill_map,
			&mut real_path_set,
	...
	for raw_path in &options.skill_paths {
		...
		if !Path::new(&resolved_path).exists() {
			all_diagnostics.push(warning("skill path does not exist".to_string(), &resolved_path));
			continue;
		}
...
mod tests {
	...
	fn loads_a_valid_skill_and_reports_no_diagnostics() {
		let root = temp_dir();
		write_skill(&root, "websearch", "name: websearch\ndescription: Search the web", "Body");
	...
	fn warns_for_invalid_names_and_skips_missing_descriptions() {
		...
		assert!(result
			.diagnostics
			.iter()
			.any(|diagnostic| diagnostic.message.contains("name contains invalid characters")));
		let long = "a".repeat(65);
		write_skill(&root, "long", &format!("name: {long}\ndescription: d"), "Body");
		let result = load_skills_from_dir(LoadSkillsFromDirOptions {
			dir: root.to_string_lossy().to_string(),
			source: "user".to_string(),
		});
		assert!(result
			.diagnostics
			.iter()
			.any(|diagnostic| diagnostic.message == format!("name exceeds 64 characters ({})", long.len())));
	...
	fn loads_nested_skills_recursively_but_prefers_a_root_skill_md() {
		...
		write_skill(&root.join("outer"), "inner", "name: inner\ndescription: d", "Body");
		let result = load_skills_from_dir(LoadSkillsFromDirOptions {
	...
	fn load_skills_detects_collisions_and_shared_python_import_names() {
		...
		write_skill(&agent_dir.join("skills"), "dup", "name: dup\ndescription: first", "Body");
		...
		assert_eq!(
			collision.collision.as_ref().unwrap().winner_path.contains("agent"),
			true
		);
		std::fs::remove_dir_all(&root).ok();
	}
	...
	fn load_skills_warns_when_python_import_names_are_shared() {
		...
		for (directory, name) in [("one", "shared-name"), ("two", "shared_name")] {
			...
			std::fs::write(skill_dir.join("pyproject.toml"), "[project]\n").unwrap();
			std::fs::write(skill_dir.join("src").join("shared_name").join("__init__.py"), "").unwrap();
			std::fs::write(
				skill_dir.join("SKILL.md"),
				format!("---\nname: {name}\ndescription: d\n---\n\nBody\n"),
			)
	...
	fn load_skills_warns_when_a_path_is_not_markdown() {
		...
		assert_eq!(result.diagnostics[0].message, "skill path is not a markdown file");
		std::fs::remove_dir_all(&root).ok();
	}
	...
	fn ignore_matcher_honours_negation_and_directory_patterns() {
		...
		matcher.add(&["node_modules/".to_string(), "*.md".to_string(), "!keep.md".to_string()]);
