...
use super::super::memory::evidence::{collect_evidence, evidence_window, serialize_evidence, AgentMessage, Evidence};
...
pub type CompletionFn = Arc<dyn Fn(RefinementCompletionRequest) -> Pin<Box<dyn Future<Output = AssistantMessage> + Send>> + Send + Sync>;
...
fn slug(raw: &str, fallback: &str) -> String {
	...
	let normalized = regex::Regex::new(r"[^a-z0-9]+").unwrap().replace_all(&normalized, "_");
	let normalized = regex::Regex::new(r"^_+|_+$").unwrap().replace_all(&normalized, "");
...
fn with_default_refinement_scope(mut result: RefinementResult, scope: HarnessScope) -> RefinementResult {
...
fn entry_from_value(id: &str, raw: &Value, scope: HarnessScope) -> Option<HarnessEntry> {
	...
	value.insert("scope".to_string(), serde_json::to_value(normalize_harness_scope(value.get("scope"), scope)).ok()?);
			...
			.and_then(Value::as_object)
		{
			for (id, raw_entry) in records {
				if let Some(entry) = entry_from_value(id, raw_entry, scope) {
					state.entries.get_mut(kind.as_str()).unwrap().insert(id.clone(), entry);
				}
			}
		}
	}
...
pub fn merge_harness_states(global_state: &HarnessState, local_state: Option<&HarnessState>) -> HarnessState {
	let mut merged = empty_harness_state();
	merged.schema = global_state.schema.max(local_state.map(|state| state.schema).unwrap_or(1.0));
	for kind in RefinementKind::ALL {
		...
		if let Some(global_entries) = global_entries {
			for (id, entry) in global_entries {
				...
				merged.entries.get_mut(kind.as_str()).unwrap().insert(id.clone(), cloned);
			}
		}
		if let Some(local_entries) = local_state.and_then(|state| state.entries.get(kind.as_str())) {
			for (id, entry) in local_entries {
				let mut cloned = clone_entry(Some(entry)).unwrap();
				let scope = normalize_harness_scope(serde_json::to_value(cloned.scope).ok().as_ref(), HarnessScope::Local);
				...
				merged.entries.get_mut(kind.as_str()).unwrap().insert(merged_id, cloned);
			}
		}
	}
	let mut refinements = global_state.refinements.clone();
	refinements.extend(local_state.map(|state| state.refinements.clone()).unwrap_or_default());
...
pub fn append_global_refinement(harness_state_dir: &str, result: &RefinementResult) -> Result<String, String> {
...
pub fn merge_refinement_history(
	...
	for result in session {
		...
		let next = match (result.scope, existing.as_ref().and_then(|value| value.scope)) {
...
fn compact_text(text: &str, max_length: usize) -> String {
	let normalized = regex::Regex::new(r"\s+").unwrap().replace_all(text, " ").trim().to_string();
...
pub fn format_refinement_notice_body(result: &RefinementResult) -> String {
	let mut lines = vec![compact_text(&result.summary, DEFAULT_OVERVIEW_CONTENT_LIMIT)];
	for edit in &result.applied_edits {
		if !edit.applied {
			continue;
		}
		...
		let title = entry.map(|entry| entry.title.clone()).unwrap_or_else(|| edit.id.clone());
...
pub fn format_harness_state_for_prompt(state: &HarnessState, options: FormatHarnessStateOptions) -> String {
	let max_entries_per_kind = options.max_entries_per_kind.unwrap_or(DEFAULT_OVERVIEW_ENTRY_LIMIT);
	let max_refinements = options.max_refinements.unwrap_or(DEFAULT_OVERVIEW_REFINEMENT_LIMIT);
	let max_content_length = options.max_content_length.unwrap_or(DEFAULT_OVERVIEW_CONTENT_LIMIT);
	let include_ipython_examples = options.include_ipython_examples.unwrap_or(true);
	let include_refine_examples = options.include_refine_examples.unwrap_or(include_ipython_examples);
	...
	for kind in RefinementKind::ALL {
		...
		for entry in entries.iter().take(max_entries_per_kind) {
			let arguments_text = if entry.kind == RefinementKind::Skill && !entry.arguments.is_empty() {
				format!(
					" args={}",
					compact_text(&serde_json::to_string(&entry.arguments).unwrap_or_default(), max_content_length)
				)
			} else {
				String::new()
			};
			let reference_text = if entry.kind == RefinementKind::Skill && !entry.reference.is_empty() {
				format!(
					" ref={}",
					compact_text(&serde_json::to_string(&entry.reference).unwrap_or_default(), max_content_length)
				)
			} else {
				String::new()
			};
		...
		let overflow = entries.len().saturating_sub(entries.len().min(max_entries_per_kind));
		if overflow > 0 {
	...
	for event in &state.refinements[start..] {
		...
		let outcome = if event.outcome.is_empty() {
			...
			format!("; outcome: {}", compact_text(&event.outcome, max_content_length))
		};
		lines.push(format!(
			"- [{}] {}: {}{}",
			event.id,
	...
	let refinement_overflow = state.refinements.len().saturating_sub(state.refinements.len().min(max_refinements));
...
fn overview_for_prompt(state: &HarnessState) -> String {
	...
	for kind in RefinementKind::ALL {
		...
		if entries.len() > 40 {
			lines.push(format!("- +{} more {} entries", entries.len() - 40, kind.as_str()));
		}
	}
	lines.join("\n")
}
...
fn normalize_python_json_literals(candidate: &str) -> String {
	...
	while index < characters.len() {
		...
		for (source, target) in replacements {
			...
			if index + source_chars.len() > characters.len() || characters[index..index + source_chars.len()] != source_chars[..] {
				continue;
			}
			let before = if index == 0 { None } else { Some(characters[index - 1]) };
...
pub fn normalize_refinement_proposal(value: &Value) -> RefinementProposal {
	...
	let edits = record.get("edits").and_then(Value::as_array).cloned().unwrap_or_default();
	...
	RefinementProposal {
		...
		edits: edits
			...
			.map(|edit| {
				...
				let kind = edit.get("kind").and_then(Value::as_str).unwrap_or("").to_string();
				let action = edit.get("action").and_then(Value::as_str).unwrap_or("").to_string();
				RefinementEdit {
					action,
					kind,
					id: edit.get("id").and_then(Value::as_str).map(str::to_string),
					title: edit.get("title").and_then(Value::as_str).map(str::to_string),
					content: edit.get("content").and_then(Value::as_str).map(str::to_string),
					...
					reason: edit.get("reason").and_then(Value::as_str).map(str::to_string),
				}
			})
			.collect(),
	}
...
pub fn validate_edit(edit: &RefinementEdit, computed_id: Option<&str>) -> Option<String> {
	...
	if kind == "prompt" && (edit_id == Some("base_system_prompt") || computed_id == Some("base_system_prompt")) {
	...
	let has_title = edit.title.as_deref().map(|value| !value.is_empty()).unwrap_or(false);
	let has_content = edit.content.as_deref().map(|value| !value.is_empty()).unwrap_or(false);
...
pub fn apply_refinement_proposal(
	...
	for edit in &proposal.edits {
		...
		if edit.action == "delete" {
			...
			state.entries.get_mut(kind.as_str()).unwrap().shift_remove(&id);
				...
				.map(|bucket| {
					bucket.values().find(|entry| {
						entry.content.trim() == edit.content.clone().unwrap_or_default().trim()
							&& entry.metadata.get("hostId") == edit.metadata.as_ref().and_then(|meta| meta.get("hostId"))
		...
		state.entries.get_mut(kind.as_str()).unwrap().insert(id.clone(), after.clone());
		proposal_modified_keys.insert(entry_key);
		applied_edits.push(AppliedRefinementEdit {
			edit: edit.clone(),
			id,
...
pub fn rollback_proposal(target: &RefinementResult) -> RefinementProposal {
	...
	for edit in target.applied_edits.iter().rev() {
		...
		match &edit.before {
			Some(before) => edits.push(RefinementEdit {
				action: if edit.after.is_some() { "update" } else { "create" }.to_string(),
	...
	RefinementProposal {
		...
		rationale: format!("Restores continual harness state snapshots from refinement {}.", target.id),
...
pub fn generate_refinement_id() -> String {
	let digits: String = now().chars().filter(|character| character.is_ascii_digit()).collect();
...
fn build_refinement_repair_prompt(original_prompt: &str, invalid_text: &str, validation_error: &str) -> String {
...
fn parse_auto_refine_review(text: &str) -> Result<AutoRefineReview, RefinementJsonError> {
	...
	Ok(AutoRefineReview {
		...
		instructions: record.get("instructions").and_then(Value::as_str).map(str::to_string),
	})
}
...
pub async fn plan_refinement(request: PlanRefinementRequest<'_>) -> Result<RefinementPlan, RefinementFailureError> {
	...
	let max_tokens = refinement_max_output_tokens(request.model)
		.min(request.options.max_output_tokens.unwrap_or(REFINEMENT_MAX_OUTPUT_TOKENS));
	let mut fingerprints: Vec<RefinementOutputFingerprint> = Vec::new();
	let mut prompt = user_prompt;
	for attempt in [1u8, 2u8] {
		let response = call_completion(&request.complete, REFINEMENT_SYSTEM_PROMPT, &prompt, max_tokens).await;
				...
				for edit in proposal.edits.iter_mut() {
						...
						Some(ids) => evidence
							...
							.filter(|source| {
								window.ids.contains(&source.id)
									&& ids.iter().any(|value| value.as_str() == Some(source.id.as_str()))
							})
							.cloned()
							.collect(),
						None => Vec::new(),
					...
					metadata.insert(
						...
						Value::Array(
							cited
								.iter()
								.map(|source| serde_json::to_value(source.source()).unwrap_or(Value::Null))
								.collect(),
						),
					);
					metadata.insert(
...
pub async fn review_auto_refine(request: ReviewAutoRefineRequest<'_>) -> Result<AutoRefineReview, RefinementJsonError> {
...
mod tests {
	...
	fn creates_ids_from_titles_and_uses_default_path_and_metadata_when_omitted() {
		...
		let entry = state.entries.get("memory").unwrap().get("use_sydney").unwrap();
	...
	fn applies_create_update_and_delete_for_every_editable_harness_kind() {
		...
		for kind in ["prompt", "memory", "skill", "subagent"] {
			state
				...
				.get_mut(kind)
				.unwrap()
				.insert("e".to_string(), serde_json::from_value(entry_value("e", kind, "t", "c")).unwrap());
		}
		let proposal = normalize_refinement_proposal(&serde_json::json!({
			"summary": "s", "rationale": "r", "expectedOutcome": "o",
			"edits": [
		...
		assert_eq!(state.entries.get("memory").unwrap().get("e").unwrap().version, 2);
	...
	fn rejects_unsupported_actions_and_kinds_and_base_system_prompt_edits() {
		...
		assert_eq!(result.applied_edits[0].error.as_deref(), Some("unsupported action replace"));
		assert_eq!(result.applied_edits[1].error.as_deref(), Some("unsupported kind unknown"));
		assert_eq!(result.applied_edits[2].error.as_deref(), Some("base system prompt is not editable"));
	...
	fn requires_python_reference_and_arguments_for_harness_created_skills() {
		...
		assert_eq!(result.applied_edits[0].error.as_deref(), Some("create skill requires arguments"));
		assert_eq!(result.applied_edits[1].error.as_deref(), Some("create skill requires python reference"));
		assert_eq!(
	...
	fn rejects_an_edit_when_the_target_entry_changed_after_planning() {
		...
		state
			...
			.get_mut("memory")
			.unwrap()
			.insert("e".to_string(), serde_json::from_value(entry_value("e", "memory", "t", "c")).unwrap());
		let proposal = normalize_refinement_proposal(&serde_json::json!({
			"summary": "s", "rationale": "r", "expectedOutcome": "o",
	...
	fn allows_sequential_edits_to_the_same_entry_after_the_baseline_matches_once() {
		...
		assert!(result.applied_edits.iter().all(|edit| edit.applied));
		assert_eq!(state.entries.get("memory").unwrap().get("e").unwrap().title, "t3");
	}
	...
	fn infers_legacy_refinement_scope_from_applied_edit_snapshots() {
		...
		proposal.edits[0].metadata = Some(serde_json::json!({"scope": "global"}).as_object().unwrap().clone());
		let result = apply_refinement_proposal(
			&mut state,
			&proposal,
			ApplyRefinementOptions {
		...
		legacy.scope = None;
		assert_eq!(infer_refinement_result_scope(&legacy), Some(HarnessScope::Global));
	}
	...
	fn uses_a_global_harness_state_directory_under_the_agent_dir_by_default() {
		...
		assert_eq!(get_harness_state_path(&dir), format!("{dir}/harness_state.json").replace('\\', "/"));
	...
	fn merges_global_and_local_harness_state_without_hiding_colliding_entries() {
		...
		assert_eq!(memory.get("shared").unwrap().scope, Some(HarnessScope::Global));
	}
	...
	fn preserves_entry_scope_stored_inside_the_global_harness_file() {
		let dir = temp_dir();
		let mut state = empty_harness_state();
		let mut entry: HarnessEntry = serde_json::from_value(entry_value("e", "memory", "t", "c")).unwrap();
		entry.scope = Some(HarnessScope::Local);
		state.entries.get_mut("memory").unwrap().insert("e".to_string(), entry);
		save_harness_state(&dir.to_string_lossy(), &state).unwrap();
		let loaded = load_harness_state(&dir.to_string_lossy(), HarnessScope::Global);
		assert_eq!(loaded.entries.get("memory").unwrap().get("e").unwrap().scope, Some(HarnessScope::Local));
		std::fs::remove_dir_all(&dir).ok();
	}
	...
	fn loads_empty_state_for_corrupt_or_missing_files() {
		...
		assert!(load_harness_state(&dir.to_string_lossy(), HarnessScope::Global)
			.entries
			.values()
	...
	fn formats_refinement_notices_and_harness_state_for_prompt() {
		let mut state = empty_harness_state();
		let mut entry: HarnessEntry = serde_json::from_value(entry_value("e", "memory", "Use Sydney", "Note")).unwrap();
	...
	fn rolls_back_created_updated_and_deleted_entries_from_refinement_history() {
		...
		state
			...
			.get_mut("memory")
			.unwrap()
			.insert("keep".to_string(), serde_json::from_value(entry_value("keep", "memory", "t", "c")).unwrap());
		let proposal = normalize_refinement_proposal(&serde_json::json!({
			"summary": "s", "rationale": "r", "expectedOutcome": "o",
			"edits": [
		...
		state
			...
			.get_mut("memory")
			.unwrap()
			.insert("gone".to_string(), serde_json::from_value(entry_value("gone", "memory", "t", "c")).unwrap());
		let result = apply_refinement_proposal(
			&mut state,
			&proposal,
			ApplyRefinementOptions {
	...
	fn parses_truncated_and_malformed_json_with_named_causes() {
		let truncated = parse_proposal("{\"summary\": \"s\", \"edits\": [{\"action\": \"create\"").unwrap_err();
	...
	fn mints_refinement_ids_in_the_canonical_format() {
		...
		assert!(id["refine_".len()..].chars().all(|character| character.is_ascii_digit()));
	}

	#[test]
	fn formats_the_repair_prompt_with_untrusted_prior_response() {
		let prompt = build_refinement_repair_prompt("original", "bad", "the model did not return valid JSON");
	...
	fn plans_a_rollback_without_mutating_harness_state() {
		...
		let plan = runtime.block_on(plan_refinement(PlanRefinementRequest {
			messages: &[],
			state: &state,
			...
			headers: None,
			thinking_level: None,
			complete: Arc::new(|_| Box::pin(async { unreachable!("no completion for rollback") })),
		}))
		.expect("plan");
		...
		assert_eq!(serde_json::to_string(&state).unwrap(), serde_json::to_string(&before).unwrap());
	}
	...
	fn reports_an_exhausted_output_budget_instead_of_a_json_parse_error() {
		...
		assert_eq!(error.refinement_failure.category, RefinementFailureCategory::Truncated);
		assert_eq!(error.refinement_failure.attempts, 1);
		assert_eq!(error.refinement_failure.output_fingerprints.len(), 1);
		assert_eq!(error.refinement_failure.output_fingerprints[0].utf8_bytes, 14);
		assert!(error
			.refinement_failure
			.output_fingerprints[0]
			.sha256
			.chars()
	...
	fn adds_global_only_scope_policy_when_planning_a_global_refinement() {
		...
		assert!(prompt.contains("<user_refine_instructions>\nbe careful\n</user_refine_instructions>"));
	...
	fn skips_paused_auto_refine_reviews_without_a_model_call() {
		...
		let review = runtime
			.block_on(review_auto_refine(ReviewAutoRefineRequest {
				...
				thinking_level: None,
				retry: None,
				complete: Arc::new(|_| Box::pin(async { unreachable!("paused review must not call the model") })),
			}))
			.expect("review");
		assert!(!review.should_refine);
		assert_eq!(review.rationale, "Automatic learning is paused for this project.");
	}
