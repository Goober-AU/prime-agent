//! Port of packages/coding-agent/src/modes/interactive/components/daxnuts.ts
//!
//! POWERED BY DAXNUTS - Easter egg for OpenCode + Kimi K2.5
//!
//! A heartfelt tribute to dax (@thdxr) for providing free Kimi K2.5 access via OpenCode.

use std::cell::RefCell;
use std::rc::Rc;
use std::sync::atomic::{AtomicBool, AtomicI64, Ordering};
use std::sync::Arc;

use pi_tui::tui::{Component, TUI};

use super::super::theme::theme::theme;

// 32x32 RGB image of dax, hex encoded (3 bytes per pixel)
const DAX_HEX: &str = "";

const WIDTH: usize = 32;
const HEIGHT: usize = 32;

fn parse_image() -> Vec<Vec<[u8; 3]>> {
    let bytes = DAX_HEX.as_bytes();
    let mut pixels: Vec<Vec<[u8; 3]>> = Vec::with_capacity(HEIGHT);
    for y in 0..HEIGHT {
        let mut row: Vec<[u8; 3]> = Vec::with_capacity(WIDTH);
        for x in 0..WIDTH {
            let idx = (y * WIDTH + x) * 6;
            let r = parse_hex_byte(&bytes[idx..idx + 2]);
            let g = parse_hex_byte(&bytes[idx + 2..idx + 4]);
            let b = parse_hex_byte(&bytes[idx + 4..idx + 6]);
            row.push([r, g, b]);
        }
        pixels.push(row);
    }
    pixels
}

/// `parseInt(value, 16)`. The port keeps the NaN case as 0, which is what the
/// `rgb(...)` template string renders for a non-numeric channel.
fn parse_hex_byte(slice: &[u8]) -> u8 {
    let text = std::str::from_utf8(slice).unwrap_or("");
    u8::from_str_radix(text, 16).unwrap_or(0)
}

fn rgb(r: u8, g: u8, b: u8, bg: bool) -> String {
    format!("\x1b[{};2;{r};{g};{b}m", if bg { 48 } else { 38 })
}

const RESET: &str = "\x1b[0m";

fn build_image() -> Vec<String> {
    let pixels = parse_image();
    let mut lines: Vec<String> = Vec::new();

    // Use half-block chars: ▄ with bg=top pixel, fg=bottom pixel
    let mut row = 0usize;
    while row < HEIGHT {
        let mut line = String::new();
        for x in 0..WIDTH {
            let top = pixels[row][x];
            let bottom = pixels.get(row + 1).map(|r| r[x]).unwrap_or(top);
            line.push_str(&rgb(bottom[0], bottom[1], bottom[2], false));
            line.push_str(&rgb(top[0], top[1], top[2], true));
            line.push('▄');
        }
        line.push_str(RESET);
        lines.push(line);
        row += 2;
    }
    lines
}

/// Port of `DaxnutsComponent`.
pub struct DaxnutsComponent {
    ui: Rc<RefCell<TUI>>,
    image: Vec<String>,
    /// Port of `setInterval`, which the TypeScript clears in `stopAnimation()`.
    interval_task: Option<tokio::task::JoinHandle<()>>,
    running: Arc<AtomicBool>,
    /// The tick counter lives in an atomic so the timer task can advance it and
    /// `render` can read it, matching `this.tick`.
    tick: Arc<AtomicI64>,
    /// `maxTicks = 25` (~2 seconds at 80ms)
    max_ticks: i64,
    cached_lines: Vec<String>,
    cached_width: f64,
    cached_tick: i64,
}

impl DaxnutsComponent {
    pub fn new(ui: Rc<RefCell<TUI>>) -> Self {
        let mut component = Self {
            ui,
            image: build_image(),
            interval_task: None,
            running: Arc::new(AtomicBool::new(false)),
            tick: Arc::new(AtomicI64::new(0)),
            max_ticks: 25,
            cached_lines: Vec::new(),
            cached_width: -1.0,
            cached_tick: -1,
        };
        component.start_animation();
        component
    }

    fn start_animation(&mut self) {
        let tick = Arc::clone(&self.tick);
        let running = Arc::clone(&self.running);
        let max_ticks = self.max_ticks;
        running.store(true, Ordering::SeqCst);
        // Port of `setInterval(() => { this.tick++; ... this.ui.requestRender(); }, 80)`.
        // The `TUI` is not `Send`, so the interval task only advances the shared
        // tick counter; the render loop reads it, exactly like the `ArminComponent`
        // and pi-tui `Loader` ports do for their animations.
        let task = tokio::spawn(async move {
            let mut ticker = tokio::time::interval(std::time::Duration::from_millis(80));
            ticker.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Delay);
            ticker.tick().await;
            while running.load(Ordering::SeqCst) {
                ticker.tick().await;
                if !running.load(Ordering::SeqCst) {
                    break;
                }
                let next = tick.load(Ordering::SeqCst) + 1;
                tick.store(next, Ordering::SeqCst);
                if next >= max_ticks {
                    running.store(false, Ordering::SeqCst);
                }
            }
        });
        self.interval_task = Some(task);
    }

    fn stop_animation(&mut self) {
        self.running.store(false, Ordering::SeqCst);
        if let Some(task) = self.interval_task.take() {
            task.abort();
        }
    }

    /// `invalidate()` - drops the width cache.
    pub fn invalidate_cache(&mut self) {
        self.cached_width = -1.0;
    }

    /// Port of `dispose()`.
    pub fn dispose(&mut self) {
        self.stop_animation();
    }
}

impl Component for DaxnutsComponent {
    fn render(&mut self, width: f64) -> Vec<String> {
        let tick = self.tick.load(Ordering::SeqCst);
        if width == self.cached_width && self.cached_tick == tick {
            return self.cached_lines.clone();
        }

        let active = theme();
        let mut lines: Vec<String> = Vec::new();

        let center = |s: &str| -> String {
            let visible = strip_sgr_len(s);
            let left = (width - visible as f64) / 2.0;
            let left = if left < 0.0 { 0 } else { left.floor() as usize };
            format!("{}{s}", " ".repeat(left))
        };

        lines.push(String::new());

        // Scanline reveal effect: show rows progressively
        let revealed_rows =
            ((tick as f64 / self.max_ticks as f64) * (self.image.len() + 3) as f64).floor();
        let revealed_rows = (revealed_rows as usize).min(self.image.len());

        for i in 0..self.image.len() {
            if i < revealed_rows {
                lines.push(center(&self.image[i]));
            } else if i == revealed_rows {
                // Show scan line
                let scanline = "▓".repeat(WIDTH);
                lines.push(center(&format!(
                    "{}{scanline}{RESET}",
                    rgb(100, 200, 255, false)
                )));
            } else {
                lines.push(center(&" ".repeat(WIDTH)));
            }
        }

        lines.push(String::new());

        // Fade in text after image is revealed
        let text_phase = (tick as f64 - self.max_ticks as f64 * 0.6).max(0.0);
        if text_phase > 0.0 || tick >= self.max_ticks {
            lines.push(center(
                &active.fg("accent", "Free Kimi K2.5 via OpenCode Zen"),
            ));
            lines.push(center(&active.fg("success", "\"Powered by daxnuts\"")));
            lines.push(center(&active.fg("muted", "— @thdxr")));
        } else {
            lines.push(String::new());
            lines.push(String::new());
            lines.push(String::new());
        }

        lines.push(String::new());
        if text_phase > 2.0 || tick >= self.max_ticks {
            lines.push(center(&active.fg("dim", "Try OpenCode")));
            lines.push(center(
                &active.fg("mdLink", "https://mistral.ai/news/mistral-vibe-2-0"),
            ));
        } else {
            lines.push(String::new());
            lines.push(String::new());
        }
        lines.push(String::new());

        self.cached_lines = lines.clone();
        self.cached_width = width;
        self.cached_tick = tick;
        lines
    }

    fn invalidate(&mut self) {
        self.invalidate_cache();
    }
}

impl Drop for DaxnutsComponent {
    fn drop(&mut self) {
        self.stop_animation();
    }
}

/// `s.replace(/\x1b\[[0-9;]*m/g, "").length` - the visible SGR-stripped length.
fn strip_sgr_len(s: &str) -> usize {
    let mut count = 0usize;
    let bytes: Vec<char> = s.chars().collect();
    let mut i = 0usize;
    while i < bytes.len() {
        if bytes[i] == '\x1b' && i + 1 < bytes.len() && bytes[i + 1] == '[' {
            let mut j = i + 2;
            while j < bytes.len() && (bytes[j].is_ascii_digit() || bytes[j] == ';') {
                j += 1;
            }
            if j < bytes.len() && bytes[j] == 'm' {
                i = j + 1;
                continue;
            }
        }
        count += 1;
        i += 1;
    }
    count
}
